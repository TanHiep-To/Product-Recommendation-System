...
class ColabFiltering:
    def __init__(self, file_path="./data/raw/ratings.csv") -> None:
        ...
        self.dataset = Dataset.load_from_df(
            self.data[['userId', 'movieId', 'rating']], self.reader
        )
        self.model = SVD()
    ...
    def train_model(self) -> None:
        ...
        if self.model is None or self.dataset is None:
            raise ValueError(
                "Model and dataset must be initialized before training."
            )
    ...
    def evaluate_model(self) -> None:
        ...
        results = cross_validate(
            self.model, self.dataset, measures=['RMSE', 'MAE']
        )
    ...
    def recommend(self, user_id: int, top_n: int = 10) -> pd.DataFrame:
        ...
        if self.model is None or self.dataset is None:
            raise ValueError(
                "Model and dataset must be initialized before making recommendations."
            )
        ...
        rated_movie_ids = self.data[self.data['userId'] == user_id][
            'movieId'
        ].unique()
        ...
        predictions = [
            self.model.predict(user_id, mid) for mid in movie_ids_to_predict
        ]
        for prediction in predictions[
            :5
        ]:  # Print first 5 predictions to check
            print(
                f"Predicted rating for movie {prediction.iid}: {prediction.est}"
            )
        ...
        recommended_movies = self.data[
            self.data['movieId'].isin(top_movie_ids)
        ].drop_duplicates('movieId')
        return recommended_movies[['movieId', 'rating']].reset_index(drop=True)
...
def main() -> None:
    ...
    for user_id in unique_user_ids[:1]:
        ...
        print(
            f"Top 10 recommendations for User ID {user_id}:\n", recommendations
        )
